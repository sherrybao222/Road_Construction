...
let expName = 'try3';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  ploygon = new visual.ShapeStim ({
    win: psychoJS.window, name: 'ploygon',
  ...
  line = new visual.ShapeStim ({
    ...
    vertices: [[-[100, 100][0]/2.0, 0], [+[100, 100][0]/2.0, 0]],
    ori: 0, pos: [0, 0],
    lineWidth: 1, lineColor: new util.Color([1, 1, 1]),
    fillColor: new util.Color([1, 1, 1]),
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    ploygon.setPos([sin1, sin2]);
    // setup some python lists for storing info about the mouse
    gotValidClick = false; // until a click is received
    polygon2.setPos(loc2);
    line.setPos([loc1, loc2]);
    // keep track of which components have finished
    trialComponents = [];
    trialComponents.push(ploygon);
    trialComponents.push(mouse);
    trialComponents.push(polygon2);
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    // *ploygon* updates
    if (t >= 0.0 && ploygon.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ploygon.tStart = t;  // (not accounting for frame time here)
      ploygon.frameNStart = frameN;  // exact frame index

      ploygon.setAutoDraw(true);
    }
