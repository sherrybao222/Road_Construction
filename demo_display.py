...
class Map:
    ...
    def dis_pair(city_a):
        for cities in map_1.all_city_xy:
            dis_check = map_1.distance(city_a, cities)
            if budget.bud_history[-1] < dis_check:
                return False
...
class Budget:
    ...
    @staticmethod
    def budget_dis():
        checked_list = list.copy(map_1.all_city_xy)
        current_dis = []
        current_loc = draw_map.collision(pg.mouse.get_pos())
        if current_loc in checked_list:
            checked_list.remove(current_loc)
        for loc in collision:
            if loc in checked_list:
                checked_list.remove(loc)
        for dis in checked_list:
            x1, y1 = collision[-1][0], collision[-1][1]
            x2, y2 = dis[0], dis[1]     # each city in the list
            distance = round(math.hypot(x1 - x2, y1 - y2), 2)
            current_dis.append(distance)
        # print(current_dis)
        # print(checked_list)
        return current_dis
    ...
    def budget_check():
        # budget_no = []
        budget_yes = []
        for dis in budget.budget_dis():
            if dis <= budget.bud_history[-1]:
                budget_yes.append(dis)
            if dis >= budget.bud_history[-1]:
                budget.budget_dis().remove(dis)
        if all(budget.budget_dis()) > budget.bud_history[-1]:
            return False
        if len(budget_yes) > 0:
            return True
...
class Data:
    ...
    def game_update():  # update with input for new draw information
        ...
        collision.append(Draw.collision(pg.mouse.get_pos()))
...
class Draw:
    ...
    def auto_snap(mouse_loc):
        budget_allow = budget.bud_history[-1] >= Map.distance(collision[-1], pg.mouse.get_pos())
        if budget_allow:
            if Draw.collision(pg.mouse.get_pos()):
                pg.draw.line(screen, BLACK, collision[-1], draw_map.collision(mouse_loc), road_size)

    @staticmethod
    def mouse_limit(mouse_loc):
        mouse_dis = Map.distance(collision[-1], mouse_loc)
        if mouse_dis < budget.bud_history[-1]:
            pg.draw.circle(screen, GREEN, pg.mouse.get_pos(), city_size)
        else:
            pg.draw.circle(screen, GREEN, Budget.budget_pos(collision[-1][0], collision[-1][1], budget.bud_history[-1]), city_size)

    @staticmethod
    def road_visual():
        pg.draw.lines(screen, BLACK, False, collision, road_size)
        pg.draw.line(screen, WHITE, click[0], city_start.xy, road_size)
        # to hide the first line draw from origin
        pg.draw.circle(screen, RED, city_start.xy, city_size)
    ...
    @staticmethod
    def game_end():
        pg.draw.rect(screen, BLACK, (300, 250, 400, 100), 0)
        Draw.text_write('Your score is ' + str(len(click) - 2), 50, WHITE, 350, 300)
        pg.display.flip()
    ...
    def __init__(self):
        ...
        # Draw.road_undo()
        # Draw.road_reset()
        self.city_visit = len(click) - 2
        Draw.text_write(('Score: ' + str(self.city_visit)), 50, BLACK, 800, 100)
        for cities in map_1.all_city_xy:
            self.city_rect = (pg.draw.circle(screen, BLACK, cities, city_size))
        if budget.bud_history[-1] > Map.distance(click[-1], pg.mouse.get_pos()):
            Draw.auto_snap(pg.mouse.get_pos())
        pg.draw.line(screen, GREEN, collision[-1], Budget.budget_pos(collision[-1][0], collision[-1][1], budget.bud_history[-1]), road_size)
    ...
    def refresh():  # append put anchor to indicate new start
        ...
        collision.append(city_start.xy)
...
while not done:
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            draw_map.__init__()
            if not budget.budget_check():
                draw_map.game_end()
        elif event.type == pg.MOUSEBUTTONDOWN:
            if draw_map.collision(pg.mouse.get_pos()) and event.button == 1:
                if budget.bud_history[-1] >= Map.distance(collision[-1], pg.mouse.get_pos()):
                    data1.game_update()
                    data1.saving()
                    draw_map.__init__()
