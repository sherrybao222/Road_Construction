...
for i,cond in enumerate(orderBlk[1]): # for now we only have this order; will need another script for another order
    if cond == 2:
...
while subCount < 1:
        ...
        for i in range(0,len(single_trial)):
            if (i==0)or(single_trial['cityNr'][i-1] != single_trial['cityNr'][i]):
                ...
                for c in remain_trial:
                    if basic_map[0][currenMapID]['distance'][int(single_trial['choiceHis'][i])][int(c)] <= float(single_trial['budgetHis'][i]):
                        n_u = n_u + 1
                        u_trial.append(c)
                n_u_all.append(n_u)
    ...
    data = {'map_id':map_id, 'choice_all':choice_all, 
