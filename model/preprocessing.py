...
for num in subs:
    ...
    for i,idx in enumerate(basic_index):
        choice_trial = all_data[0][idx]['choice_dyn'][1:-1]
        choice_next = all_data[0][idx]['choice_dyn'][2:]
        choice_all.extend(choice_trial)
        choice_next_all.extend(choice_next)
        
        budget_trial = all_data[0][idx]['budget_dyn'][0:-2]
        budget_all.extend(budget_trial)
    ...
    data = {'move_id':move_id, 'map_id':map_id, 'choice_all':choice_all, 
            ...
            'n_city_all':n_city_all,'chosen_all':chosen_all, 'remain_all':remain_all}
    # Create DataFrame 
    df = pd.DataFrame(data) 
